...
%% Output
function [accuracyMean,accuracyStd,F_LCAMean,FHMean,TIEmean] = FS_Kflod_TopDownSVMClassifier_plane(data, numFolds, tree, feature, numberSel,indices)
...
%             trainData = sparse(trainDataMod{i});
            ...
            modelSVM = svmtrain(trainLabel, trainData(:,selFeature), '-c 1 -t 0 -q');
...
%     [predict_label] = FS_topDownSVMPrediction(test_data, modelSVM, tree,feature,numberSel) ;
    [predict_label] = svmpredict(test_data(:,end),test_data(:,selFeature), modelSVM,'-q');
